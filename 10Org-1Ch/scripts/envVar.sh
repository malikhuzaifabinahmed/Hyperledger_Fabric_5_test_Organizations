...
setGlobalsCLI() {
  ...
  if [ $USING_ORG -eq 1 ]; then
    ...
    export CORE_PEER_ADDRESS=peer0.org5.example.com:12051
  elif [ $USING_ORG -eq 6 ]; then
    ...
    export CORE_PEER_ADDRESS=peer0.org9.example.com:16051
  elif [ $USING_ORG -eq 10 ]; then
    export CORE_PEER_ADDRESS=peer0.org10.example.com:17051
  else
    errorln "ORG Unknown"
  fi
}
