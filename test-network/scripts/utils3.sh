...
function printHelp() {
  ...
  if [ "$USAGE" == "up" ]; then
    ...
    println "    -c3 <channel name> - Name of channel to create (defaults to \"mychannel\")"
    ...
    println "   \033[0;32mup createChannel3\033[0m -ca -c -r -d -s -i -cai -verbose"
    println
    println " Examples:"
    println "   network.sh up createChannel3 -ca -c mychannel -s couchdb -i 2.0.0"
  elif [ "$USAGE" == "createChannel3" ]; then
    println "Usage: "
    println "  network.sh \033[0;32mcreateChannel3\033[0m [Flags]"
    println
    println "    Flags:"
    println "    -c3 <channel name> - Name of channel to create (defaults to \"mychannel\")"
    println "    -r <max retry> - CLI times out after certain number of attempts (defaults to 5)"
    ...
    println
    println " Possible Mode and flag combinations"
    println "   \033[0;32mcreateChannel3\033[0m -c -r -d -verbose"
    println
    println " Examples:"
    println "   network.sh createChannel3 -c channelName"
  elif [ "$USAGE" == "deployCC3" ]; then
    println "Usage: "
    println "  network.sh \033[0;32mdeployCC3\033[0m [Flags]"
    println
    println "    Flags:"
    println "    -c3 <channel name> - Name of channel to deploy chaincode to"
    ...
    println
    println " Possible Mode and flag combinations"
    println "   \033[0;32mdeployCC3\033[0m -ccn -ccl -ccv -ccs -ccp -cci -r -d -verbose"
    println
    println " Examples:"
    println "   network.sh deployCC3 -ccn basic -ccp ../asset-transfer-basic/chaincode-javascript/ ./ -ccl javascript"
    println "   network.sh deployCC3 -ccn mychaincode -ccp ./user/mychaincode -ccv 1 -ccl javascript"
  else
    println "Usage: "
    println "  network.sh <Mode> [Flags]"
    println "    Modes:"
    println "      \033[0;32mup\033[0m - Bring up Fabric orderer and peer nodes. No channel is created"
    println "      \033[0;32mup createChannel3\033[0m - Bring up fabric network with one channel"
    println "      \033[0;32mcreateChannel\033[0m - Create and join a channel after the network is created"
    println "      \033[0;32mdeployCC3\033[0m - Deploy a chaincode to a channel (defaults to asset-transfer-basic)"
    println "      \033[0;32mdown\033[0m - Bring down the network"
    println
    println "    Flags:"
    println "    Used with \033[0;32mnetwork.sh up\033[0m, \033[0;32mnetwork.sh createChannel3\033[0m:"
    println "    -ca <use CAs> -  Use Certificate Authorities to generate network crypto material"
    println "    -c3 <channel name> - Name of channel to create (defaults to \"mychannel\")"
    println "    -s <dbtype> - Peer state database to deploy: goleveldb (default) or couchdb"
    println "    -r <max retry> - CLI times out after certain number of attempts (defaults to 5)"
    ...
    println "    -verbose - Verbose mode"
    println
    println "    Used with \033[0;32mnetwork.sh deployCC3\033[0m"
    println "    -c3 <channel name> - Name of channel to deploy chaincode to"
    println "    -ccn <name> - Chaincode name."
    ...
    println
    println " Examples:"
    println "   network.sh up createChannel3 -ca -c mychannel3 -s couchdb -i 2.0.0"
    println "   network.sh createChannel3 -c channelName"
    println "   network.sh deployCC3 -ccn basic -ccp ../asset-transfer-basic/chaincode-javascript/ -ccl javascript"
    println "   network.sh deployCC3 -ccn mychaincode -ccp ./user/mychaincode -ccv 1 -ccl javascript"
  fi
