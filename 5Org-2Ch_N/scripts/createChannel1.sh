...
createChannel() {
	...
	while [ $rc -ne 0 -a $COUNTER -lt $MAX_RETRY ] ; do
		sleep $DELAY
		set -x
		peer channel create -o localhost:7050 -c $CHANNEL_NAME_1 --ordererTLSHostnameOverride orderer.example.com -f ./channel-artifacts/${CHANNEL_NAME_1}.tx --outputBlock $BLOCKFILE --tls --cafile $ORDERER_CA >&log.txt
		res=$?
		{ set +x; } 2>/dev/null
		let rc=$res
...
setAnchorPeer() {
  ORG=$1
  docker exec cli ./scripts/setAnchorPeer1.sh $ORG $CHANNEL_NAME_1 
}

FABRIC_CFG_PATH=${PWD}/configtx

## Create channeltx
infoln "Generating channel create transaction '${CHANNEL_NAME_1}.tx'"
createChannelTx

FABRIC_CFG_PATH=$PWD/../config/
BLOCKFILE="./channel-artifacts/${CHANNEL_NAME_1}.block"

## Create channel
infoln "Creating channel ${CHANNEL_NAME_1}"
createChannel
successln "Channel '$CHANNEL_NAME_1' created"
